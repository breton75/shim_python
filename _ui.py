...
## -> фильтрация ##
	...
	frameFilter.grid(row=0, column=1, sticky=tk.W)
...
## -> преобразование шим ##
	...
	frameShim.grid(row=1, column=0, sticky=tk.W)
...
## -> выгрузка на устройство ##
	...
	frameSend.grid(row=1, column=1, sticky=tk.W)
	...
	# IP
	lblIP = tk.Label(frameSend, text='IP', width=25).grid(row=1, column=0, sticky=tk.E)
	editIP = tk.Entry(frameSend, width=16)
	editIP.grid(row=1, column=1, sticky=tk.W)
	if 'ip' in ini: editIP.insert(0, ini['ip'])
	else: editIP.insert(0, '172.16.4.55')
...
## -> отрисовка ##
	...
	framePlot.grid(row=2, column=0, sticky=tk.W)
	...
	# сигнал с точки .. по ..
	lblPlotFromPoint = tk.Label(framePlot, text='Выводить сигнал с точки', width=25).grid(row=1, column=0, sticky=tk.E)
	editPlotFromPoint = tk.Entry(framePlot, width=5)
	editPlotFromPoint.grid(row=1, column=1, sticky=tk.W)
	if 'plotfrompoint' in ini: editPlotFromPoint.insert(0, ini['plotfrompoint'])
	else: editPlotFromPoint.insert(0, '1')
	
	lblPlotToPoint = tk.Label(framePlot, text=' по ', width=4).grid(row=1, column=2, sticky=tk.E)
	editPlotToPoint = tk.Entry(framePlot, width=5)
	editPlotToPoint.grid(row=1, column=3, sticky=tk.W)
	...
	checkPlotFiltered.grid(row=2, column=0, sticky=tk.W, columnspan=4)
	
	# отобразить спектр сигнала
	checkSignalSpectrum = tk.Checkbutton(framePlot, text='Сспектр сигнала')
	checkSignalSpectrum.grid(row=3, column=0, sticky=tk.W, columnspan=4)
	...
	checkPlotFilteredSignalSpectrum.grid(row=4, column=0, sticky=tk.W, columnspan=4)

	# отобразить ШИМ
	checkPlotShim = tk.Checkbutton(framePlot, text='ШИМ')
	checkPlotShim.grid(row=5, column=0, sticky=tk.W, columnspan=4)
